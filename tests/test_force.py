...
def test_tensile_normal_threshold_is_1_5_newtons():
    # A reverse reaction within the configured allowance is accepted.
    step(NormalAdmittance(), force=[0, 0, -1.5])
    # Exceeding it stops before any admittance displacement is applied.
    core = NormalAdmittance()
    with pytest.raises(ForceSafetyStop, match="negative normal force"):
        step(core, force=[0, 0, -1.51])
    assert core.offset_m == 0 and core.velocity_m_s == 0
